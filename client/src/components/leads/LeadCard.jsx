...
const LeadCard = ({ lead, onDelete, onConvert }) => {
  ...
  return (
    <div className="glass-card p-4 sm:p-5 flex flex-col hover:border-neon-green/30 hover:shadow-[0_0_30px_rgba(0,255,136,0.1)] transition-all duration-300 group">

      {/* ── Header: avatar + name + status ─────────────────────────────── */}
      <div className="flex items-start justify-between mb-4 gap-2">
        <div className="flex items-center gap-3 min-w-0">
          {/* Avatar */}
          <div className="w-11 h-11 flex-shrink-0 rounded-xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 flex items-center justify-center border border-purple-500/20">
          ...
          <div className="min-w-0">
            <h3
              ...
              className="font-semibold text-white group-hover:text-neon-green transition-colors cursor-pointer truncate max-w-[160px] sm:max-w-none"
              title={lead.leadName}
            >
              {lead.leadName}
            </h3>
            <p className="text-xs text-gray-400 truncate max-w-[160px] sm:max-w-none" title={lead.businessName}>
      ...
      <div className="grid grid-cols-1 gap-2 mb-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="flex items-center gap-1.5 text-sm text-gray-400 min-w-0">
            <span className="flex-shrink-0">{sourceIcons[lead.source] || "📌"}</span>
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400 min-w-0">
          ...
          {!lead.organization && !lead.createdBy && (
            <span className="ml-auto flex-shrink-0 px-2 py-0.5 rounded-full text-[10px] font-medium bg-purple-500/20 text-purple-400 border border-purple-500/30 whitespace-nowrap">
              Website
            </span>
          )}
        </div>
      ...
      {lead.estimatedValue > 0 && (
        <div className="mb-4 p-3 rounded-xl bg-neon-green/5 border border-neon-green/20">
          ...
          <p className="text-base font-semibold text-neon-green mt-0.5">
